...
function ResultPage() {
  ...
  return (
    <AppShell>
      <div className="mb-6 rounded-2xl border border-border bg-gradient-to-bl from-primary/10 to-card p-8 text-center shadow-elegant">
        ...
        <div className="mt-6 flex justify-center gap-3">
          <Link to="/app" className="rounded-lg border border-border bg-card px-5 py-2 text-sm">الرئيسية</Link>
        </div>
      </div>

      <h2 className="mb-3 text-lg font-semibold">مراجعة الأسئلة</h2>
                ...
                {choices.map((c: any) => {
                  ...
                  return (
                    <div key={c.id} className={`rounded-lg border p-2.5 text-sm ${c.is_correct ? "border-success/40 bg-success/5" : sel ? "border-destructive/40 bg-destructive/5" : "border-border"}`}>
                      {c.text}
                      {c.is_correct && <span className="mr-2 text-xs text-success">✓ الإجابة الصحيحة</span>}
                      {sel && !c.is_correct && <span className="mr-2 text-xs text-destructive">إجابتك</span>}
                    </div>
                  );
                })}
              </div>
