...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "تسجيل الدخول — Medical Space" },
      { name: "description", content: "سجّل دخولك أو أنشئ حساباً جديداً للوصول إلى منصة Medical Space." },
    ],
  }),
  component: AuthPage,
});
...
function AuthPage() {
  ...
  const google = async () => {
    ...
    try {
      ...
      toast.error(err?.message ?? "فشل تسجيل الدخول عبر Google");
      setLoading(false);
    }
  };

  return (
    <div className="bg-hero flex min-h-screen items-center justify-center px-4" dir="rtl">
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8 shadow-elegant">
          <h1 className="text-2xl font-bold">{mode === "signin" ? "تسجيل الدخول" : "إنشاء حساب جديد"}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {mode === "signin" ? "أهلاً بعودتك، أدخل بياناتك" : "ابدأ رحلتك التعليمية الآن"}
          </p>
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === "signup" && (
              <div>
                <label className="mb-1 block text-xs text-muted-foreground">الاسم الكامل</label>
                <input
                  required
                  value={fullName}
            ...
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">كلمة المرور</label>
              <input
                type="password"
                required
                minLength={6}
            ...
            >
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : mode === "signin" ? "تسجيل الدخول" : "إنشاء حساب"}
            </button>
          </form>
          ...
          >
            {mode === "signin" ? "لا تملك حساباً؟ أنشئ حساباً جديداً" : "لديك حساب؟ سجّل الدخول"}
          </button>
        </div>
      </div>
    </div>
